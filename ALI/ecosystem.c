...
/* Soft population cap: stop natural reproduction above this many live fish,
 * so the tank can grow a bit but never explodes into a slow, crashing mess. */
#define REPRO_POPULATION_CAP 90

void spawn_baby_fish(Fish *dad, Fish *mom)
{
    if (!dad || !mom || !g_aquarium)
        return;
    if (count_alive_fish() >= REPRO_POPULATION_CAP)
        return; /* tank full enough -- skip this birth */
