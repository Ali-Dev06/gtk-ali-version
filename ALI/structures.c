...
void ProgramStart(){
    /* Force the X11 (Xwayland) backend under WSL. The standard window
       minimize/maximize/close buttons only work correctly there: on WSLg's
       Wayland backend the native maximize triggers a fatal protocol error.
       Native Linux is left on its normal backend. */
    if (g_getenv("WSL_DISTRO_NAME") || g_getenv("WSL_INTEROP")) {
        /* Prefer X11 (Xwayland) so the native maximize works, but fall back to
           Wayland if Xwayland isn't running -- otherwise the app would refuse
           to start with "cannot open display :0". */
        gdk_set_allowed_backends("x11,wayland");
        /* Xwayland under WSLg has no default X cursor theme, so the pointer
           would otherwise go invisible over the window. */
